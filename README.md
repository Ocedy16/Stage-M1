...
The ground truth for paths and cycles in a graph are calculated thanks to formulas located in `libs/countsub.py`. The ground truth for induced paths are hard coded in the `libs/ground_truth.py` file. They have been calculated thanks to the function count_induced_paths in `libs/generate_truth.py`.

## Hyperparameters

There are a lot of hyperparameters in the different code files. The optimal ones need to be found in order for the program to work better. 
Here are a list of the most important ones and what they represent :

- max_depth : max size of each word.
- nb_word : number of words generated by the MCTS to find the formulas. It needs to be kept at 1 for the code to work. It could only be modified with Piquenot's code, before changes.
- batch_size : the batch size for learning.
- value_confidence : how much the MCTS will take the trained model into account to make its decisions.
- search_param : the 'C' in the MCTS method. If search_param is high, the algorithm will prioritize exploration over exploitation.
- bad_leaf : -bad_leaf is attributed to the formulas that must be rejected by the algorithm and aren't exploitable.
- In the result function of the `MCTS.py` file, the value next to nb is a hyperparameter that sets the range covered by the results. If it's low then the results have a high range and a slightly better formula leads to great score increases whereas a high hyperparameter limits the score jumps.
- action_cost in `acting.py` defines the costs of the operations. It has been reduced to not have a lot of impact on the results. Initially they were in the .01 range.
